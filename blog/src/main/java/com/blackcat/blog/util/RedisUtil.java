...
public class RedisUtil {
	 ...
	 * @return T key对应的值
	 */
	public <T> T get(Object key,Class<?> t){
	 ...
	 * @return void
	*/
	public void deleteSub(String subName,Object... key){
		log.info("delete(...) => key -> {}", key);
		try {
			Boolean result;
			if (ArrayUtils.isEmpty(key)) {
				if (ArrayUtils.getLength(key) == 1) {
					...
					redisTemplate.delete(StringUtils.split(idsString,","));
				}
			}
		} catch (Exception e) {
			log.error("删除一个或多个key",e);
	 ...
	 * @return
	 */
	public boolean hmset(Object key, Map<String, Object> map, long time, TimeUnit unit) {
		try {
			redisTemplate.opsForHash().putAll(key, map);
			if (time > 0) {
	 ...
	 * @return
	 */
	public boolean hset(Object key, String item, Object value, long time, TimeUnit unit) {
		try {
			redisTemplate.opsForHash().put(key, item, value);
			if (time > 0) {
				setExpire(key, time, unit);
	 ...
	 * @return 成功个数
	 */
	public long sSetAndTime(Object key, long time, TimeUnit unit, Object... values) {
		try {
	 ...
	 * @return
	 */
	public Set<Object> zSGet(Object key) {
		try {
			return redisTemplate.opsForSet().members(key);
		} catch (Exception e) {
			log.error(key.toString(), e);
	 ...
	 * @return true 存在 false不存在
	 */
	public boolean zSHasKey(Object key, Object value) {
		try {
			return redisTemplate.opsForSet().isMember(key, value);
		} catch (Exception e) {
			log.error(key.toString(), e);
	...
	public Boolean zSSet(Object key, Object value, double score) {
		try {
	 ...
	 * @return 成功个数
	 */
	public long zSSetAndTime(Object key, long time, TimeUnit unit, Object... values) {
		try {
			Long count = redisTemplate.opsForSet().add(key, values);
			if (time > 0) {
				setExpire(key, time, unit);
	 ...
	 * @return
	 */
	public long zSGetSetSize(Object key) {
		try {
			return redisTemplate.opsForSet().size(key);
		} catch (Exception e) {
			log.error(key.toString(), e);
	 ...
	 * @return 移除的个数
	 */
	public long zSetRemove(Object key, Object... values) {
		try {
			Long count = redisTemplate.opsForSet().remove(key, values);
			return count;
		} catch (Exception e) {
	 ...
	 * @return
	 */
	public boolean lSet(Object key, Object value, long time, TimeUnit unit) {
		try {
			redisTemplate.opsForList().rightPush(key, value);
			if (time > 0) {
				setExpire(key, time, unit);
	 ...
	 * @return
	 */
	public boolean lSet(Object key, List<Object> value, long time, TimeUnit unit) {
		try {
			redisTemplate.opsForList().rightPushAll(key, value);
			if (time > 0) {
				setExpire(key, time, unit);
